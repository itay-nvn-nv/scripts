...
for iw in "${pending_iws[@]}"; do
  ...
  # Find PlacementGroups with the matching workloadId annotation
  ...
  echo "$pods"
  echo "PlacementGroups to be deleted:"
  echo "$pgs"
  ...
  # Delete the placementgroups
  if [ -n "$pgs" ]; then
    echo "Deleting PlacementGroups..."
    kubectl delete pg -n "$namespace" $(echo "$pgs")
  else
    echo "No PlacementGroups found to delete."
  fi
